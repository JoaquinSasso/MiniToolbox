...
class UploadMetricsWorker(appContext: Context, params: WorkerParameters) :
    ...
    override suspend fun doWork(): Result = withContext(Dispatchers.IO) {
        ...
        if (payloadJson.isBlank()) {
            ...
            // También se sanean: un lote congelado por una versión anterior puede contener
            // claves inválidas, y commitSent las escribiría de vuelta en los mapas de
            // enviados, reintroduciendo el problema que el sanitizador acaba de corregir.
            deltas = parseDeltasFromPendingPayload(payloadJson).mapNotNull { sanitizeDelta(it) }
        }
