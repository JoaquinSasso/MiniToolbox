...
fun ProScreen(onBack: () -> Unit) {
    ...
    val billingWrapper = remember { BillingClientWrapper(context, scope) }


    LaunchedEffect(Unit) {
        billingWrapper.initialize()
    }
    DisposableEffect(Unit) {
        onDispose { billingWrapper.destroy() }
    }
    ...
    LaunchedEffect(purchaseStatus) {
        ...
        } else if (purchaseStatus is PurchaseStatus.Failed) {
    ...
    val isBuyButtonEnabled = productDetails != null && !isPurchasing

    Scaffold(
        topBar = {
            TopBarReusable(
                title = stringResource(R.string.pro_screen_title),
                onBack = onBack
            )
        }
    ) { padding ->
        Column(
            ...
            // Icono de la sección PRO
            Icon(
                imageVector = Icons.Default.WorkspacePremium,
                contentDescription = stringResource(R.string.pro_feature_icon),
                tint = Color(0xFFFFD700), // Color dorado (hex #FFD700)
            ...
            // Lista de beneficios
            BenefitItem(
                icon = Icons.Default.WebAssetOff,
            ...
            // Botones de acción
            Button(
                onClick = { /* TODO: Iniciar flujo de compra */ },
                ...
                if (isPurchasing) {
                    ...
                    Text(stringResource(R.string.pro_buy_button, productPrice))
                }
            }
            Spacer(Modifier.height(8.dp))
            TextButton(onClick = { /* TODO: Restaurar compras */ }) {
